...
const fetchCoffeeStores = async (latLong="43.66302728887845,-79.40072517262301", limit=6) => {
...
export const fetchCoffeeShopsData = async (latlong, limit) => {
  ...
  for await (const store of coffeeStores) {
    ...
    const imgUrl = imgData?.prefix && imgData.prefix+'original'+imgData.suffix;
    data.push({ imgUrl, ...store });
  }
